...
double dotGridGradient(int ix, int iy, double x, double y)
{
    ...
    //TODO: Compute and return the dot-product
     // Get gradient vector at grid node
...
double ray_sphere_intersection(const Vector3d &ray_origin, const Vector3d &ray_direction, int index, Vector3d &p, Vector3d &N)
{
    ...
    p = ray_origin;
    N = ray_direction;
    Vector3d oc = p - sphere_center;
	double a = N.dot(N);
	double b = 2.0 * oc.dot(N);
...
double ray_parallelogram_intersection(const Vector3d &ray_origin, const Vector3d &ray_direction, int index, Vector3d &p, Vector3d &N)
{
    ...
    Vector3d solution = M.fullPivLu().solve(rhs);
    double u = solution(0);
    double v = solution(1);
    double t = solution(2);
	double epsilon = 1e-4;

    if (u >= 0 && u <= 1 && v >= 0 && v <= 1 && t >= epsilon) {
		p = ray_origin + t * ray_direction;
        N = pgram_u.cross(pgram_v).normalized();
        if (N.dot(-ray_direction) < 0) N = -N;
        return t;
    }
...
bool is_light_visible(const Vector3d &ray_origin, const Vector3d &ray_direction, const Vector3d &light_position)
{
    ...
    Vector3d shadow_origin = ray_origin + epsilon * ray_direction;
    Vector3d shadow_dir = (light_position - shadow_origin).normalized();
    double light_distance = (light_position - shadow_origin).norm();

    Vector3d shadow_p, shadow_N;
    int shadow_obj = find_nearest_object(shadow_origin, shadow_dir, shadow_p, shadow_N);

    if (shadow_obj >= 0) {
        double t = (shadow_p - shadow_origin).norm();
        if (t < light_distance)
...
Vector4d shoot_ray(const Vector3d &ray_origin, const Vector3d &ray_direction, int max_bounce)
{
    ...
    for (int i = 0; i < light_positions.size(); ++i)
    {
        ...
        if (is_light_visible(p, N, light_position)) {
            ...
            Vector3d h = (v + Li) / (v + Li).norm();
		    const Vector4d specular = obj_specular_color * std::pow(std::max(N.dot(h), 0.0), obj_specular_exponent);
    ...
    if (max_bounce > 0) {
        const double epsilon = 1e-4;
        Vector3d reflection_origin = p + epsilon * N;
        Vector3d reflection_dir = ray_direction - 2 * N.dot(ray_direction) * N;
        reflection_dir.normalize();
        reflection_color = refl_color.cwiseProduct(shoot_ray(reflection_origin, reflection_dir, max_bounce - 1));
    }
...
void raytrace_scene()
{
    ...
    double image_y = focal_length * tan(field_of_view / 2.0);
